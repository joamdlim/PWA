...
import {
  ...
  syntaxTree,
  tags
} from "./chunk-OOZHQYZM.js";
import {
  EditorSelection,
  EditorView,
  Prec,
  keymap
} from "./chunk-NSZKCNR5.js";
...
var Parse = class {
  ...
  advanceStack(stack, stacks, split) {
    ...
    if (defaultReduce > 0) {
      ...
      if (verbose)
        console.log(base + this.stackID(stack) + ` (via always-reduce ${parser5.getName(defaultReduce & 65535)})`);
      return true;
    }
    ...
    for (let i = 0; i < actions.length; ) {
      ...
      if (verbose)
        console.log(base + this.stackID(localStack) + ` (via ${(action & 65536) == 0 ? "shift" : `reduce of ${parser5.getName(action & 65535)}`} for ${parser5.getName(term)} @ ${start}${localStack == stack ? "" : ", split"})`);
      if (last)
        return true;
...
function mkLang(parser5) {
  return new Language(data, parser5);
}
...
function getContext(node, line, doc) {
  ...
  for (let i = nodes.length - 1; i >= 0; i--) {
    ...
    if (node2.name == "Blockquote" && (match = /^[ \t]*>( ?)/.exec(line.slice(pos)))) {
    ...
    } else if (node2.name == "ListItem" && node2.parent.name == "OrderedList" && (match = /^([ \t]*)\d+([.)])([ \t]*)/.exec(nodeStart(node2, doc)))) {
    ...
    } else if (node2.name == "ListItem" && node2.parent.name == "BulletList" && (match = /^([ \t]*)([-+*])([ \t]{1,4}\[[ xX]\])?([ \t]+)/.exec(nodeStart(node2, doc)))) {
      ...
      pos += len;
      context.push(new Context(node2.parent, start, pos, match[1], after, type, node2));
    }
  }
  return context;
}
...
function markdown(config = {}) {
  let { codeLanguages, defaultCodeLanguage, addKeymap = true, base: { parser: parser5 } = commonmarkLanguage } = config;
